...
// Internal representation in the last 46-bits of the u64
#[repr(transparent)]
...
impl InputState {
    ...
    pub fn edges_toward(&self, next: Self) -> impl Iterator<Item = (KeyPosition, KeyEvent)> {
        self.into_iter()
            .zip(next.into_iter())
...
impl Iterator for InputStateIterator {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if index == BIT_ORDER.len() {
            None
        } else {
            self.offset += 1;
            Some((BIT_ORDER[index], self.state.is_set(BIT_ORDER[index])))
        }
    }
}
...
impl IntoIterator for InputState {
    type Item = (KeyPosition, bool);
    type IntoIter = InputStateIterator;
