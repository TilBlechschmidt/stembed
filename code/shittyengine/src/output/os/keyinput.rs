// This code has been adapted from the crate autopilot-rs by Michael Sanders.
//
// As the library contains a lot of other stuff and pulls in many other dependencies,
// the necessary parts have been extracted in the form of this file originating from:
// https://github.com/autopilot-rs/autopilot-rs/blob/master/src/key.rs
//
...
//
//! This module contains functions for simulating keypresses on desktop operating systems.
#![allow(dead_code)]
...
/// Attempts to simulate typing a string
pub fn type_string(string: &str, flags: &[Flag]) {
...
impl From<KeyCode> for CGKeyCode {
    fn from(code: KeyCode) -> CGKeyCode {
        match code {
            ...
            KeyCode::Num8 => 0x1C,
            KeyCode::Num9 => 0x19,
            KeyCode::NumDecimal => 0x41,
            KeyCode::NumLock => 0x47,
            KeyCode::NumDivide => 0x4B,
            KeyCode::NumMultiply => 0x43,
            KeyCode::NumSubtract => 0x4E,
            KeyCode::NumAdd => 0x45,
            KeyCode::NumEnter => 0x4C,
            KeyCode::Insert => unimplemented!(),
        }
    }
}
...
impl From<KeyCode> for WinKeyCode {
    fn from(code: KeyCode) -> WinKeyCode {
        ...
        let win_code = match code {
            ...
            KeyCode::Num0 => winuser::VK_NUMPAD0,
            KeyCode::Num1 => winuser::VK_NUMPAD1,
            KeyCode::Num2 => winuser::VK_NUMPAD2,
            KeyCode::Num3 => winuser::VK_NUMPAD3,
            KeyCode::Num4 => winuser::VK_NUMPAD4,
            KeyCode::Num5 => winuser::VK_NUMPAD5,
            KeyCode::Num6 => winuser::VK_NUMPAD6,
            KeyCode::Num7 => winuser::VK_NUMPAD7,
            KeyCode::Num8 => winuser::VK_NUMPAD8,
            KeyCode::Num9 => winuser::VK_NUMPAD9,
            KeyCode::NumDecimal => winuser::VK_DECIMAL,
            KeyCode::NumLock => winuser::VK_NUMLOCK,
            KeyCode::NumDivide => winuser::VK_DIVIDE,
            KeyCode::NumMultiply => winuser::VK_MULTIPLY,
            KeyCode::NumSubtract => winuser::VK_SUBTRACT,
            KeyCode::NumAdd => winuser::VK_ADD,
            KeyCode::NumEnter => winuser::VK_RETURN,
        };
        win_code as WinKeyCode
    }
}
...
fn system_toggle<T: KeyCodeConvertible>(key: &T, down: bool, flags: &[Flag]) {
    ...
    for &flag in flags.iter() {
        win_send_key_event(WinKeyCode::from(flag), down);
    }
    if let Some(character) = key.character() {
        let flags = if down { 0 } else { KEYEVENTF_KEYUP };
        let mut buf = [0; 2];
        for word in character.encode_utf16(&mut buf) {
            let mut input = INPUT {
                ...
                u: unsafe {
                    core::mem::transmute_copy(&KEYBDINPUT {
                        wVk: 0,
                        wScan: *word,
            ...
            unsafe {
                SendInput(1, &mut input, core::mem::size_of::<INPUT>() as i32);
            }
        }
    } else {
...
impl From<KeyCode> for XKeyCode {
    fn from(code: KeyCode) -> XKeyCode {
        let x_code = match code {
            ...
            KeyCode::Num0 => x11::keysym::XK_KP_0,
            KeyCode::Num1 => x11::keysym::XK_KP_1,
            KeyCode::Num2 => x11::keysym::XK_KP_2,
            KeyCode::Num3 => x11::keysym::XK_KP_3,
            KeyCode::Num4 => x11::keysym::XK_KP_4,
            KeyCode::Num5 => x11::keysym::XK_KP_5,
            KeyCode::Num6 => x11::keysym::XK_KP_6,
            KeyCode::Num7 => x11::keysym::XK_KP_7,
            KeyCode::Num8 => x11::keysym::XK_KP_8,
            KeyCode::Num9 => x11::keysym::XK_KP_9,
            KeyCode::NumDecimal => x11::keysym::XK_KP_Decimal,
            KeyCode::NumLock => x11::keysym::XK_Num_Lock,
            KeyCode::NumDivide => x11::keysym::XK_KP_Divide,
            KeyCode::NumMultiply => x11::keysym::XK_KP_Multiply,
            KeyCode::NumSubtract => x11::keysym::XK_KP_Subtract,
            KeyCode::NumAdd => x11::keysym::XK_KP_Add,
            KeyCode::NumEnter => x11::keysym::XK_KP_Enter,
        };
        XKeyCode::from(x_code)
    }
}
...
#[cfg(target_os = "linux")]
fn x_send_key_event(display: *mut x11::xlib::Display, keycode: XKeyCode, down: bool) {
    unsafe {
        XTestFakeKeyEvent(
            display,
...
fn system_toggle<T: KeyCodeConvertible>(key: &T, down: bool, flags: &[Flag]) {
    internal::X_MAIN_DISPLAY.with(|display| {
        for &flag in flags.iter() {
            x_send_key_event(display.as_ptr(), XKeyCode::from(flag), down);
        }
        x_send_key_event(display.as_ptr(), key.code(), down);
    })
}
